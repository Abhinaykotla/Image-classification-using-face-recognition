...
image_count = 0
def send_alert():
    if dirName_base == None and dirName_output == None and fileName == None:
        messagebox.showerror('Missing Inputs','Please give all inputs')
    
    else:
        if dirName_base == None:
            messagebox.showerror('Missing Inputs','Please select a base folder')
        elif dirName_output == None :
            messagebox.showerror('Missing Inputs','Please select a output folder')
        else :
            messagebox.showerror('Missing Inputs','Please select the target image')

def clear_inputs():
    if dirName_base == None and dirName_output == None and fileName == None:
        messagebox.showinfo('Already clear','No inputs given')
    else:
        dirName_base == None
        dirName_output == None
        fileName == None
        file_explorer_base_show = Label(root, text='', width=100, height=1, 
                anchor='w', fg="red", bg='black', font=('Helvetica', 12, 'italic'))
        file_explorer_base_show.place(x=90, y=100)
        file_explorer_output_show = Label(root, text='', width=100, 
                            height=1, anchor='w', fg="red", bg='black', font=('Helvetica', 12, 'italic'))
        file_explorer_output_show.place(x=90, y=200)
        #cleared alert
        messagebox.showwarning('Alert!','Cleared all inputs')
...
def open_dir_base():
    ...
    if dir != None:
        ...
        global file_explorer_base_show
        file_explorer_base_show = Label(root, text='Chosen path: ' + dirName_base, width=100, height=1, 
            anchor='w', fg="red", bg='black', font=('Helvetica', 12, 'italic'))
        file_explorer_base_show.place(x=90, y=100)
        ...
        image_count = len([name for name in listdir(dirName_base) if name.endswith(
            '.jpg' or '.png' or '.jpeg' or '.JPG' or '.PNG' or '.JPEG' or '.jfif' or '.JFIF' 
            or '.tiff' or '.TIFF' or '.bmp' or '.BMP' or '.webp' or '.WEBP')])
...
def open_dir_output():
    ...
    if dir != None:
        ...
        global file_explorer_output_show
        file_explorer_output_show = Label(root, text='Chosen path: ' + dirName_output, width=100, 
                        height=1, anchor='w', fg="red", bg='black', font=('Helvetica', 12, 'italic'))
        file_explorer_output_show.place(x=90, y=200)


def start():
    if dirName_base == None or dirName_output == None or fileName == None:
        send_alert()
        print('Please select all the files')
    else:
        # create progress bar
        global progress_bar
        progress_bar = ttk.Progressbar(root, variable=progress_var, maximum=image_count, mode='determinate', 
                                        length=980)
        print(image_count)
...
def create_copy(base_img_path, folder_selected, output_foulder):
    ...
    for image in listdir(folder_selected):
        ...
        if (image.endswith('.jpg' or '.png' or '.jpeg' or '.JPG' or '.PNG' or '.JPEG' or '.jfif' or '.JFIF' 
                            or '.tiff' or '.TIFF' or '.bmp' or '.BMP' or '.webp' or '.WEBP')):
            print(image)
...
Text = Label(root, text="Photo segregation made easier ", width=60, height=1, font=("Helvetica", 20), 
            fg="white", bg="black")
Text.place(x=0, y=0)

# Base folder select text
file_explorer_base = Label(root, text="Select base folder", width=15, height=2, anchor='w', fg="white", 
...
file_explorer_output = Label(root, text="Select output folder",  width=15, height=2, 
                        anchor='w', fg="white", bg='black', font=('Helvetica', 12, 'bold'))
...
file_explorer_file = Label(root, text="Select Image with target face", width=23, height=2, 
                        anchor='w', fg="white", bg='black', font=('Helvetica', 12, 'bold'))
file_explorer_file.place(x=10, y=250)

# Target image select button
Button(root, text="Choose Image", command=open_file).place(x=10, y=300)
...
#clear button
Button(root, text="Clear inputs", command=clear_inputs).place(x=10, y=450)
